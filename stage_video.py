...
from ffmpeg_compat import CREATE_NO_WINDOW, Capabilities, _run
...
def _temporal_chain(caps: Capabilities, low_cpu: bool, in_lbl: str) -> Tuple[str, str, Dict]:
    ...
    if caps.has("tmix"):
        if caps.tmix_weights:
            frag = f"[{cur}]tmix=frames=3:weights='1 2 1'[v1]"
            meta["tmix_weights"] = True
        else:
            frag = f"[{cur}]tmix=frames=3[v1]"
        meta["tmix"] = True
        parts = [frag]
        cur = "v1"
...
def _finalize_chain(in_lbl: str, low_cpu: bool, caps: Capabilities) -> Tuple[str, str, Dict]:
    ...
    if caps.has("noise"):
        ...
        meta["noise"] = strength
        frag = f"[{cur}]noise=alls={strength}:allf=t+u[vn]"
        cur = "vn"
    # PTS restart + SAR reset => exact 30fps stream, zero A/V drift.
    frag2 = f"[{cur}]format=yuv420p,setsar=1,setpts=PTS-STARTPTS[vout]"
    return f"{frag};{frag2}", "vout", meta
