...
# ----------------------------------------------------------------------
# L1 + L2: Constant‑time swept EQ (firequalizer) or static equalizer
# ----------------------------------------------------------------------
def _static_eq() -> str:
    """Static 3-band parametric EQ using `equalizer` (not aequalizer) 
    for maximum compatibility across FFmpeg builds."""
    return (
...
def build_audio_filtergraph(
    ...
    # - Otherwise fall back to static equalizer (which uses `equalizer` filter).
    # ------------------------------------------------------------------
